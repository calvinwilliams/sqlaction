...
public class UserOrderSAO {
	...
	*/
	public static int SELECT_u_O_name_j_u_O_address_j_o_O_ALL_FROM_user_base_u_j_user_order_o_WHERE_u_O_name_E_AND_u_O_id_E_o_O_user_id( Connection conn, List<UserBaseSAU> userBaseListForSelectOutput, List<UserOrderSAU> userOrderListForSelectOutput, String _1_UserBaseSAU_name ) throws Exception {
		PreparedStatement prestmt = conn.prepareStatement( UserOrderSAU.statementInterceptorForQueryUserAndOrderByName("SELECT u.name,u.address,o.* FROM user_base u,user_order o WHERE u.name=? AND u.id=o.user_id", _1_UserBaseSAU_name) ) ;
		prestmt.setString( 1, _1_UserBaseSAU_name );
		ResultSet rs = prestmt.executeQuery() ;
		while( rs.next() ) {
			UserBaseSAU userBase = new UserBaseSAU() ;
			...
			userBase.name = rs.getString( 1 ) ;
			userBase.address = rs.getString( 2 ) ;
			userOrder.id = rs.getInt( 3 ) ;
			userOrder.userId = rs.getInt( 4 ) ;
			userOrder.itemName = rs.getString( 5 ) ;
			userOrder.amount = rs.getInt( 6 ) ;
			userOrder.totalPrice = rs.getDouble( 7 ) ;
			userBaseListForSelectOutput.add(userBase) ;
			userOrderListForSelectOutput.add(userOrder) ;
		}
		rs.close();
	...
	public static int SELECT_o_O_ALL_FROM_user_order_o( Connection conn, List<UserOrderSAU> userOrderListForSelectOutput ) throws Exception {
		Statement stmt = conn.createStatement() ;
		ResultSet rs = stmt.executeQuery( "SELECT o.* FROM user_order o" ) ;
		while( rs.next() ) {
			UserOrderSAU userOrder = new UserOrderSAU() ;
			userOrder.id = rs.getInt( 1 ) ;
			userOrder.userId = rs.getInt( 2 ) ;
