...
public class UserOrderSAO {
	...
	*/
	...
	public static int SELECT_ALL_FROM_user_order_WHERE_user_id_E_( Connection conn, List<UserOrderSAU> userOrderListForSelectOutput, int _1_userId ) throws Exception {
	...
	public static int SELECT_ALL_FROM_user_order_PAGEKEY_id( Connection conn, List<UserOrderSAU> userOrderListForSelectOutput, int _1_pageSize, int _2_pageNum ) throws Exception {
		PreparedStatement prestmt = conn.prepareStatement( "SELECT * FROM user_order ORDER BY id OFFSET ? ROWS FETCH NEXT ? ROWS ONLY" ) ;
	...
	public static int SELECT_ALL_FROM_user_order_WHERE_item_name_NE__PAGEKEY_id( Connection conn, List<UserOrderSAU> userOrderListForSelectOutput, int _1_pageSize, int _2_pageNum ) throws Exception {
		PreparedStatement prestmt = conn.prepareStatement( "SELECT * FROM user_order WHERE item_name<>'' ORDER BY id DESC OFFSET ? ROWS FETCH NEXT ? ROWS ONLY" ) ;
		prestmt.setInt( 1, _1_pageSize*(_2_pageNum-1) );
		prestmt.setInt( 2, _1_pageSize );
		ResultSet rs = prestmt.executeQuery() ;
		while( rs.next() ) {
	...
	public static int queryUserAndOrderByName( Connection conn, List<UserBaseSAU> userBaseListForSelectOutput, List<UserOrderSAU> userOrderListForSelectOutput, String _1_name ) throws Exception {
	...
	*/
	...
	public static int INSERT_INTO_user_order( Connection conn, UserOrderSAU userOrder ) throws Exception {
		...
		rs = stmt.executeQuery( "SELECT NEXT VALUE FOR user_order_seq_id" ) ;
		rs.next();
		userOrder.id = rs.getInt( 1 ) ;
		rs.close();
		stmt.close();
		
		prestmt = conn.prepareStatement( "INSERT INTO user_order (id,user_id,item_name,amount,total_price) VALUES (?,?,?,?,?)" ) ;
		prestmt.setInt( 1, userOrder.id );
		prestmt.setInt( 2, userOrder.userId );
		prestmt.setString( 3, userOrder.itemName );
		prestmt.setInt( 4, userOrder.amount );
		prestmt.setDouble( 5, userOrder.totalPrice );
